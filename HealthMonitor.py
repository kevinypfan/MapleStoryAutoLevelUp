...
class HealthMonitor:
    ...
    '''
    def __init__(self, cfg, args, keyboard_controller):
        self.cfg = cfg
        self.args = args
        self.kb = keyboard_controller  # Use shared keyboard controller
        self.running = False
        self.enabled = True
        self.thread = None
    ...
    def _heal(self):
        '''
        Execute heal action using shared keyboard controller
        '''
        try:
            self.kb.set_command("heal")
        except Exception as e:
            logger.error(f"Heal action failed: {e}")
    
    def _add_mp(self):
        '''
        Execute MP recovery action using shared keyboard controller
        '''
        try:
            self.kb.set_command("add mp")
        except Exception as e:
